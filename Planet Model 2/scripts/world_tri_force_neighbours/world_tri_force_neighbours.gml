...
var _count = world_tri_get_count();
for( var _i = 0; _i < _count; _i++ ) {
    ...
    var _a_on_face = world_tri_find_point( _grid,   _ax, _ay, _az,   _i );
    var _b_on_face = world_tri_find_point( _grid,   _bx, _by, _bz,   _i );
    var _c_on_face = world_tri_find_point( _grid,   _cx, _cy, _cz,   _i );
	
    //If the two vertices are A+B then side A is common
    if ( _a_on_face and _b_on_face ) {
		_grid[# _current, world_tri.a_adj ] = _i;
        _result++; if ( _result == 3 ) exit;
    }
    
    //If the two vertices are B+C then side B is common
    if ( _b_on_face and _c_on_face ) {
		_grid[# _current, world_tri.b_adj ] = _i;
        _result++; if ( _result == 3 ) exit;
    }
    
    //If the two vertices are C+A then side C is common
    if ( _c_on_face and _a_on_face ) {
		_grid[# _current, world_tri.c_adj ] = _i;
        _result++; if ( _result == 3 ) exit;
    }
